...
[![Dev version](https://img.shields.io/badge/devel%20version-1.0.0.9000-blue.svg)](https://github.com/grendar/wmwAUC)
[![CRAN
...
test in fact assesses the set $\lbrace\mathrm{(F,G)\colon AUC = 0.5}\rbrace$,
which does not match $\mathrm{H_0\colon F = G}$. This incoherence [@grendar2026dct] causes
...
-   *Bias-Corrected* (BC) Method: Alternative method using individual 
    ...
    finite-sample corrections and Welch-Satterthwaite degrees of freedom. Derived for continuous  data (Sect. 4 of the paper); the
...
corresponding asymptotic theory, we demonstrate the mismatch between the null 
of the WMW test and the statistic-implied null set directly:
...
$\mathrm{H_1\colon AUC \neq 0.5}$. The following MC simulation demonstrates the 
consistency.
