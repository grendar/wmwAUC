...
#' @param pvalue_method Character, method ('EU', 'BC') used for computing p-values; 'BC' assumes continuous data (default 'EU')
#' @param ci_method Character, confidence interval method for eAUC: c('hanley', 'boot', 'none')
#' @param conf_level Numeric, confidence level for intervals (default 0.95)
#' @param n_grid Numeric, number of grid points for search in `pseudomedian_ci()` (default 100)
...
#' equals the probability \eqn{P(X > Y)} that a randomly selected observation from the first group exceeds a randomly 
...
#' 1) Exact Unbiased ('EU') estimation of variance of eAUC which handles any type of data
#' (continuous, discrete, mixed);
#' 2) Bias Correction ('BC') sample-size dependent method
#' to maintain proper Type I error control. Confidence intervals for the pseudomedian
...
#'   \item{pseudomedian}{Hodges-Lehmann median difference estimate (when special_case = TRUE)}
#'   \item{pseudomedian_conf_int}{Confidence interval for the location shift (when special_case = TRUE)}
#'   \item{pseudomedian_conf_level}{Confidence level for the confidence interval for HL estimator (when special_case = TRUE)}
...
#'   \item{auc_conf_int}{Confidence interval for true AUC using Hanley-McNeil or bootstrap method}
...
#' \code{\link{wmw_pvalue}} for details on computing p-values in the continuous case ('BC')
#' \code{\link{wmw_pvalue_ties}} for details on computing p-values in the 'EU' mode
...
#' \code{\link{wilcox.test}} for Wilcoxon-Mann-Whitney test in base R.
#'
#' @export
wmw_test <- function(formula,
  ...
  if (pvalue_method == 'EU') {
    #
    p_value <- wmw_pvalue_ties(x_vals, y_vals, alternative = alternative) 
    # NOTE: inside the fnc P(X>Y) is turned to P(X<Y) to conform with preprint math
  } else {
    has_ties <- any(table(pooled) > 1)
    if (has_ties)  {
      cat('WARNING: there are ties in data. BC method assumes no ties.\n')
    }
    # compute pval
    p_value <- wmw_pvalue(x_vals, y_vals, alternative = alternative)
    #
  }
  #
  ...
  if (special_case == TRUE) {
    ...
    #    
    pseudo_median <-  pseudomedian_ci(x_vals, y_vals, conf.level = conf_level, 
                                      pvalue_method = pvalue_method, n_grid = n_grid) 
    pseudomedian <- pseudo_median$estimate
