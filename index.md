...
$`\mathrm{H_0\colon AUC = 0.5}`$ based on the WMW statistic; for
details, see ([Grendár 2025](#ref-grendar2025wmw)). The package provides
both asymptotic inference and two finite-sample bias-correction methods:
...
are not retained as functional aliases. Calling any of them raises an
informative error naming the current function to use instead.)
...
![](reference/figures/README-plot_ex1-1.png)
...
that the classical WMW test’s calibration cannot be tracking
distributional equality.

### Simulation 1

Consider the setting of two zero-mean different-scale gaussians. Then
the traditional $`\mathrm{H_0\colon F = G}`$ of the classical WMW test
is false and $`\mathrm{H_0\colon F \neq G}`$ holds.
...
hypothesis ($`\mathrm{H_0\colon AUC = 0.5}`$) using miscalibrated
variance estimation.
...
![](reference/figures/README-hist_sim1_1-1.png)

Empirical AUC centered at 0.5 despite $`\mathrm{F \neq G}`$.

![](reference/figures/README-hist_sim1_2-1.png)

Traditional p-values under $`\mathrm{H_1}`$ should concentrate near 0.

![](reference/figures/README-hist_sim1_3-1.png)
...
![](reference/figures/README-hist_sim2-1.png)![](reference/figures/README-hist_sim2-2.png)![](reference/figures/README-hist_sim2-3.png)
